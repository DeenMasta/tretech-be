...
class ReturnSessionService
{
    ...
    /**
     * Complete a return session and auto-create + finalize a reconciliation in one step.
     */
    public function completeWithReconciliation(ReturnSession $session, User $actor): ReturnSession
    {
        return DB::transaction(function () use ($session, $actor) {
            // Step 1: Complete the return session
            $completed = $this->complete($session, $actor);

            // Step 2: Auto-create the reconciliation
            $reconciliation = $this->reconciliationService->create([
                'return_session_id' => $completed->id,
                'pic_user_id'       => $actor->id,
                'remarks'           => null,
            ], $actor);

            // Step 3: Auto-finalize the reconciliation
            $this->reconciliationFinalizeService->finalize($reconciliation, $actor);

            return $completed->refresh()->load([
                'consignment:id,consignment_no',
                'picUser:id,full_name',
                'completedByUser:id,full_name',
                'reconciliation:id,reconciliation_no,status',
            ]);
        });
    }

    /**
     * Reopen a completed return session, removing the auto-generated reconciliation.
     */
    public function reopenWithReconciliation(ReturnSession $session, string $reason, User $actor): ReturnSession
    {
        return DB::transaction(function () use ($session, $reason, $actor) {
            ...
            if ($locked->status !== 'completed') {
                throw new BusinessLogicException('Only completed return sessions can be reopened.');
            }

            if ($locked->reconciliation) {
                // 1. Find all lot movements generated by this reconciliation to revert quantities and states
                $movements = LotMovement::query()
                    ->where('reference_type', Reconciliation::class)
                    ->where('reference_id', $locked->reconciliation->id)
                    ->orderByDesc('id') // revert in reverse order just in case
                    ->get();

                foreach ($movements as $movement) {
                    $lot = \App\Models\Lot::query()->lockForUpdate()->find($movement->lot_id);
                    if (!$lot) continue;
                    ...
                    if ($movement->movement_type === 'returned') {
                        ...
                        // Delete the auto-generated holding record
                        \App\Models\LotHolding::query()
                            ->where('lot_id', $lot->id)
                    ...
                    // Revert the status and location
                ...
                // 2. Delete the reconciliation items and the reconciliation
                ReconciliationItem::query()->where('reconciliation_id', $locked->reconciliation->id)->delete();
                $locked->reconciliation->delete();
            }

            // 3. Mark return session as in_progress
            $locked->fill([
                'status'               => 'in_progress',
                'completed_at'         => null,
            ...
            $this->auditLogService->logModelAction(
                auditableType: ReturnSession::class,
                auditableId:   $locked->id,
                actionType:    AuditAction::RETURN_SESSION_REOPENED ?? 'return_session_reopened',
                actor:         $actor,
                description:   "Return session {$locked->return_session_no} reopened. Reason: {$reason}",
                after: [
                    'status' => 'in_progress',
                    'reason' => $reason,
                ],
